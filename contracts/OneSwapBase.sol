...
contract OneSwapRoot is IOneSwapView {
    ...
    IPancakeswapV2Factory internal constant pancakeswapV2 = IPancakeswapV2Factory(0xcA143Ce32Fe78f1f7019d7d551a6402fC5350c73);
    IStableSwapCalculator internal constant stableswapCalculator = IStableSwapCalculator(0x4b1bFb6f5E2B10770F08e2496E8d0CdB2e682798);
...
contract OneSwapViewWrapBase is IOneSwapView, OneSwapRoot {
    function getExpectedReturn(
        ...
        (returnAmount, , distribution) = this.getExpectedReturnWithGas(srcToken, dstToken, amount, parts, flags, 0);
    }
...
contract OneSwapView is IOneSwapView, OneSwapRoot {
        ...
        )
    {
        ...
        function(IERC20, IERC20, uint256, uint256, uint256) view returns (uint256[] memory, uint256)[DEXES_COUNT]
        ...
        )
    {
        ...
        return [
            invert != flags.check(FLAG_DISABLE_STABLESWAP_ALL | FLAG_DISABLE_STABLESWAP)
                ? _calculateNoReturn
                : calculateStableSwap,
            invert != flags.check(FLAG_DISABLE_PANCAKESWAP_V2_ALL | FLAG_DISABLE_PANCAKESWAP_V2)
                ? _calculateNoReturn
                : calculatePancakeswapV2
        ];
    }
    ...
    function _getStableSwapPoolInfo(IStableSwap stableswap)
        internal
        view
        returns (
        ...
        )
    {
        ...
        for (uint256 k = 0; k < 2 && balances[k] > 0; k++) {
    ...
    function _linearInterpolation100(uint256 value, uint256 parts) internal pure returns (uint256[100] memory rets) {
        for (uint256 i = 0; i < parts; i++) {
            rets[i] = value.mul(i + 1).div(parts);
        }
    }
    ...
    function calculateStableSwap(
        ...
        for (uint i = 0; i < pairLength; i++) {
            ...
            uint256[] memory _rets = _calculateStableSwapSelector(
                srcToken,
                dstToken,
                amount,
                parts,
    ...
    function calculatePancakeswapV2(
        ...
        return _calculatePancakeswapV2(srcToken, dstToken, _linearInterpolation(amount, parts), flags);
    }
...
contract OneSwapBaseWrap is IOneSwap, OneSwapRoot {
    function _swap(
        ...
        _swapFloor(srcToken, dstToken, amount, distribution, flags);
    }
...
contract OneSwap is IOneSwap, OneSwapRoot {
    ...
    function swap(
        ...
        require(
            ...
            "OneSwap: Distribution array should not exceed reserves array size"
        );
    ...
    function _swapOnStableSwap(
        ...
        for (uint i = 0; i < pairLength; i++) {
            ...
            tokens[0] = IERC20(stableSwap.coins(uint(0)));
            tokens[1] = IERC20(stableSwap.coins(uint(1)));
            uint i = (srcToken == tokens[0] ? 1 : 0) + (srcToken == tokens[1] ? 2 : 0);
            uint j = (dstToken == tokens[0] ? 1 : 0) + (dstToken == tokens[1] ? 2 : 0);
    ...
    function _swapOnPancakeswapV2(
        ...
        _swapOnPancakeswapV2Internal(srcToken, dstToken, amount, flags);
    }
}
